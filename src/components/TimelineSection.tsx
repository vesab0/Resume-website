        ...
        <article key={item.title} className="space-y-1">
          {(item.title || item.logo) && (
            <div className="grid grid-cols-1 gap-x-3 gap-y-1 md:grid-cols-[minmax(0,1fr)_auto] md:items-center">
              <div className="flex min-w-0 items-center gap-2">
                ...
                <p className="text-[13px] font-semibold tracking-[0.06em] text-zinc-200 md:text-[14px]">
                  {item.title}
                </p>
              </div>
              {item.date && (
                <p className="whitespace-normal md:whitespace-nowrap text-[12px] font-semibold tracking-[0.06em] text-zinc-300 md:text-[13px]">
                  {item.date}
                </p>
              )}
            </div>
          ...
          {(item.subtitle || item.location) && (
            <div className="flex flex-col gap-y-1 italic md:flex-row md:items-baseline md:justify-between md:gap-x-4">
              {item.subtitle ? (
                <p className="text-[13px] tracking-[0.06em] text-zinc-300 md:text-[14px]">
                  {item.subtitle}
                </p>
              ) : (
                <span />
            ...
            <ul className="list-disc space-y-1.5 pl-6 pt-1 marker:text-zinc-300">
              {item.details.map((detail) => (
                <li
                  ...
                  className="text-[13px] leading-relaxed tracking-[0.04em] text-zinc-300 md:text-[14px]"
                >
                  {detail}
                </li>
              ))}
